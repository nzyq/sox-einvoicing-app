...
# Successful Deletion of Invoice
def test_delete_invoice(clear, create_user):
    invoice_id = requests.post(other.UPLOAD_INVOICE_URL, json={'user_id': create_user, 'data': "Data for Invoice"}).json()['invoice_id']
    request_data = requests.delete(other.DELETE_INVOICE_URL, json={'user_id': create_user, 'invoice_id': invoice_id})
    ...
    assert (len(user_invoices) == 0)
    assert (status == 200)


# Successful Deletion from multiple invoices
def test_delete_invoice_multiple_invoices(clear, create_user):
...
def test_invalid_user_id(clear, create_user):
    invoice_id = requests.post(other.UPLOAD_INVOICE_URL, json={'user_id': create_user, 'data': "Data for Invoice"}).json()['invoice_id']
    request_data = requests.delete(other.DELETE_INVOICE_URL, json={'user_id': 2, 'invoice_id': invoice_id})
    status = request_data.status_code
...
def test_invalid_invoice_id(clear, create_user):
    request_data = requests.delete(other.DELETE_INVOICE_URL, json={'user_id': create_user, 'invoice_id': 2})
    status = request_data.status_code
...
def test_user_no_access(clear, create_user, create_user_2):
    invoice_id = requests.post(other.UPLOAD_INVOICE_URL, json={'user_id': create_user, 'data': "Data for Invoice"}).json()['invoice_id']
    request_data = requests.delete(other.DELETE_INVOICE_URL, json={'user_id': create_user_2, 'invoice_id': invoice_id})
    status = request_data.status_code
    assert (status == 403) # Access error
